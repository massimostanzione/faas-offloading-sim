...
from mab_automated_experiments._internal.graphs import graph_rewparconf
...
def mainn():
    ...
    for strat in strategies:
        ...
        if strat=="UCBTuned":
            ...
            parnames=[conf.MAB_UCB_EXPLORATION_FACTOR, confp.MAB_UCB2_ALPHA]
            efrange=np.arange(0, 1+0.25, 0.25)
                ...
                for seed in seeds:
                    for ef in efrange:
                        for param in paramrange:
                            ...
                            label=(parnames[0])+"="+str(ef)
                            if not monoparam:
                                label+=", "+(parnames[1])+"="+str(param)
