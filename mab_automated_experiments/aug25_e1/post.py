...
records=extract_datarecords_from_exp_name("aug25_e1")
print(len(records))
...
with open(os.path.join(SCRIPT_DIR, "output","rewards.txt"), "w+") as rewfile:
    for r in records:
        rewfile.write(r.identifiers["strategy"]+"\t"+
...
+"_rates.iat")
    ...
    if is_strategy_RTK(r.identifiers["strategy"]):
        ...
        instances = ["uno"]*(len(mem))#extract_result_dict_from_datarecord(r, "instance_invoked")
...
#    plt.show()
    ...
    if True:
        if os.path.exists(path):
            ...
            tempi_n2_impliciti = np.linspace(tempi_n1.min(), tempi_n1.max(), len(tempi_transizioni))
            tempi_destinazione = np.linspace(tempi_n1.min(), tempi_n1.max(), len(tempi_n1))
    ...
    plt.suptitle(r.identifiers["strategy"]+" ("+r.identifiers["axis_pre"]+") "+", "+r.identifiers["specfile"]+", scenario="+r.identifiers["mab-rtk-contextual-scenarios"])
