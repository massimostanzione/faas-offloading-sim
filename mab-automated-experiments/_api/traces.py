...
#
#square PERIOD = 10*60*60#s #sinus160*60*2#1800*4 # Periodo dell'onda sinusoidale in secondi (es. 1800 secondi = 30 minuti)
##########################PERIOD = (3*60*60)/2#linear10*60*60#s #sinus160*60*2#1800*4 # Periodo dell'onda sinusoidale in secondi (es. 1800 secondi = 30 minuti)
#PERIOD=2*SEMIPERIOD if DISTRIBUTION is "square-wave" else  2*SEMIPERIOD
#if DISTRIBUTION is "square-wave" :PERIOD*=2

# TODO oss.: se .5*pi ottengo una campana
###########################FREQ = 1 / PERIOD if DISTRIBUTION not in ["sinusoid", "shifted-sinusoid"] else 2 / PERIOD * 1 * np.pi  # Frequenza della sinusoide (ciclo completo ogni PERIOD secondi)

#########################TRACE_DURATION = PERIOD*2  # Durata totale della simulazione (3 periodi della sinusoide)

# step è l'intervallo tra un arrivo e il successivo
# quindi più piccolo è, più accurato è
#STEP_LEN = 20#sinus5*60  # Lunghezza di ogni step (es. 1800/30 = 60 secondi)
#SAMPLES_PER_PERIOD = 100000  # Numero desiderato di campioni per periodo
########################STEP_LEN = 5#PERIOD / SAMPLES_PER_PERIOD
...
def _generate_square_wave_inverted(i, min_rate, max_rate, period, step_len):
    ...
    #return generate_square_wave(i, max_rate, min_rate, period)
...
def _graph(interarrivals, rates, duration, step_len, distribution, file_path):
    ...
    # Grafico dei rates
    axs[1].plot(np.arange(STEPS) * step_len, rates, label=f"Arrival Rate (per {step_len}s)", marker="o", color="r")#, drawstyle='steps-post')
    ...
    # TODO vals è tempi
    # TODO  è valori
    # Grafico dell'onda (ideale + arrivi)
    axs[2].plot(vals, label=f"onda", marker="o", color="r", drawstyle='steps-post')
    axs[2].set_xlabel("Time (minutes)")
    axs[2].set_ylabel(f"Arrival Rate")
    axs[2].set_title(f"*{distribution}* Arrival Rate Over Time")
    axs[2].legend()
    axs[2].grid(True)
    """
    plt.tight_layout()
...
def generate_trace(name:str, distribution:str, min_rate:float, max_rate:float, period:float, step_len:float, duration:float):
    #if len(sys.argv) < 2:
    ...
    if os.path.exists(path):
        ...
        #distribution = "sawtooth-wave"#sys.argv[1].lower()
        ...
        _graph(inter_arrival_times, rates, duration, step_len, distribution, file_path)#f"../_traces/img/{name}_plot.png")
